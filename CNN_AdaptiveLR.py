...
from keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Dropout, SimpleRNN, BatchNormalization
...
for i in range(len(images)):
    if labels[i] == "positive":
        ...
        brightness = 20
        contrast = 1
        images.append(cv2.addWeighted(images[i], contrast, np.zeros(images[i].shape, images[i].dtype), 0, brightness))
        labels.append("positive")

        images.append(cv2.addWeighted(flipped, contrast, np.zeros(flipped.shape, flipped.dtype), 0, brightness))
        labels.append("positive")
...
model.add(Dropout(0.15))  # Increased dropout to prevent overfitting
model.add(Dense(2, activation='sigmoid'))  # 2-class output layer
...
# Train the model
training = model.fit(x_train, y_train, epochs=12, batch_size=64, validation_data=(x_test, y_test), callbacks=lr_scheduler)
...
model.save('SMILEs_CNN_AdaptiveLR_Dataset_Increased_80K.h5')
